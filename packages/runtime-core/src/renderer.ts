...
function baseCreateRenderer(
  ...
  const patchElement = (
    ...
    if (patchFlag > 0) {
      ...
      // cuixin: 快路径更新，不用做完整 diff，只更新这些确定会变的地方。
  ...
  const updateComponent = (n1: VNode, n2: VNode, optimized: boolean) => {
    ...
    if (shouldUpdateComponent(n1, n2, optimized)) {
      if (
        ...
        // normal update
        // cuixin: 如果需要更新，则将新节点 vnode 赋值给 next
        instance.next = n2
        // instance.update is the reactive effect.
        // cuixin: 执行前面定义在 instance 上的 update 函数。
        instance.update()
      }
    } else {
      // no update needed. just copy over properties
      // cuixin: 如果不需要更新，则将旧节点的内容更新到新节点上即可
      n2.el = n1.el
      instance.vnode = n2
    }
  }
  ...
  const setupRenderEffect: SetupRenderEffectFn = (
    ...
    const componentUpdateFn = () => {
      if (!instance.isMounted) {
        ...
        patch(
          ...
          // cuixin: 处理 teleport 相关的更新
          hostParentNode(prevTree.el!)!,
          // anchor may have changed if it's in a fragment
          // cuixin: 处理 fragment 相关的更新
    ...
    // cuixin: 创建组件的副作用渲染函数
    // 打开当前组件实例的 scope（作用域），允许在该作用域内注册响应式依赖。Vue 的 scope 用于管理响应式副作用，确保在组件销毁时能够正确清理
    instance.scope.on()
    // 创建一个 ReactiveEffect 实例，并将其赋值给组件实例的 effect 属性，Vue 响应式系统的核心类，用于追踪依赖并在依赖变化时触发更新。
    // componentUpdateFn: 这是组件的更新逻辑函数，负责处理组件的挂载和更新。
    const effect = (instance.effect = new ReactiveEffect(componentUpdateFn))
    // 关闭当前组件实例的 scope，停止注册新的响应式依赖。确保 ReactiveEffect 的创建过程是受控的，不会意外地注册额外的依赖。
    instance.scope.off()

    // 将 effect.run 方法绑定到 instance.update，以便在组件更新时调用。作用: 将 effect.run 方法绑定到 instance.update，以便在组件更新时调用。
    // effect.run: 触发 ReactiveEffect 的执行，运行 componentUpdateFn
    const update = (instance.update = effect.run.bind(effect))
    // 作用: 将 effect.runIfDirty 方法绑定到 instance.job，用于调度更新任务。
    // effect.runIfDirty: 仅在依赖发生变化时才会触发更新，避免不必要的重复执行。
    const job: SchedulerJob = (instance.job = effect.runIfDirty.bind(effect))
    // 作用: 为调度任务 job 绑定组件实例 instance 和唯一标识符 uid。
    // 背景: 这些信息在调度器中用于区分不同的任务。
    job.i = instance
    job.id = instance.uid
    // 作用: 为 ReactiveEffect 设置调度器函数。
    // queueJob(job): 将更新任务加入到任务队列中，确保更新是异步且批量执行的。
    effect.scheduler = () => queueJob(job)

    // allowRecurse
    // #1801, #2043 component render effects should allow recursive updates
    // 作用: 允许组件的渲染副作用函数在更新过程中递归调用。
    // 背景: 避免在某些生命周期钩子中递归调用导致的无限循环。
    toggleRecurse(instance, true)
    ...
    // 作用: 立即执行 update 方法，触发组件的首次渲染。
    // 背景: 这是组件挂载流程的最后一步，确保组件内容被正确渲染到页面上。
    update()
  }
  ...
  const patchChildren: PatchChildrenFn = (
    ...
    // cuixin c1 代表旧节点的子节点元素
    const c1 = n1 && n1.children
    const prevShapeFlag = n1 ? n1.shapeFlag : 0
    // cuixin c2 代表新节点的子节点元素
    ...
    if (shapeFlag & ShapeFlags.TEXT_CHILDREN) {
      ...
      if (c2 !== c1) {
        // cuixin: 新旧节点都是文本，则替换文本内容
        hostSetElementText(container, c2 as string)
      }
    } else {
      // cuixin: 新节点不为文本，旧节点是数组
      if (prevShapeFlag & ShapeFlags.ARRAY_CHILDREN) {
  ...
  const patchKeyedChildren = (
    ...
    // 5. unknown sequence 属性更新 > 位置移动 > 增删节点
    ...
    // i = 2, e1 = 4, e2 = 5
      ...
      // 5.2 loop through old children left to be patched and try to patch，最长公共子序列
      // matching nodes & remove nodes that are no longer present
      // cuixin: 寻找旧节点在新节点中对应的位置信息，如果找到则做更新，找不到则移除。
      let j
      let patched = 0
      ...
      // cuixin： 根据 newIndexToOldIndexMap 求取c
